...
{
    public partial class Form_BorrowEdit : Form
    {
        public Form_BorrowEdit()
        {
            InitializeComponent();
        }
        ...
        public Form_BorrowEdit(object[] param)
        {
            InitializeComponent();
            initEdit(param);    //初始化信息
        }
        ...
        private void EditData()
        {
            ...
            if (this.dtBack.Enabled)
            {
                sql = string.Format(@"update borrow set BookID='{0}',BookName='{1}',BookWriter='{2}',Outdate='{3}',YHdate='{4}',Indate='{5}',Fine={6},CLState='{7}',MID='{8}'
                                        where ReaID = {9}", bookId, bookName, bookWriter, dtBorrow, dtYingBack, dtBack, fineMoney, state, manageId, readerId);
            }
            else 
            {
                sql = string.Format(@"update borrow set BookID='{0}',BookName='{1}',BookWriter='{2}',Outdate='{3}',YHdate='{4}',Fine={5},CLState='{6}',MID='{7}'
                                        where ReaID = {8}", bookId, bookName, bookWriter, dtBorrow, dtYingBack, fineMoney, state, manageId, readerId);
            }
